...
export default function AnalyzePage() {
  ...
  useEffect(() => {
    ...
    try {
      // First check if we have a header in the URL
      const headerText = searchParams.get("header")

      // If no header in URL, check sessionStorage (for direct route)
      const storedHeader = typeof window !== "undefined" ? sessionStorage.getItem("emailHeader") : null

      if (!headerText && !storedHeader) {
        // If no header is provided anywhere, redirect to sample page
        router.push("/sample")
        return
      }
      ...
      if (headerText) {
        // Try to decode from URL parameter
        try {
          decodedHeader = decodeURIComponent(headerText)
        } catch (decodeError) {
          console.error("Error decoding header from URL:", decodeError)
          setError("The email header contains characters that cannot be processed via URL. Please use the form again.")
          setLoading(false)
          return
        }
      } else {
        // Use the stored header directly (no need to decode)
        decodedHeader = storedHeader

        // Clear the stored header to prevent it from being used again
        if (typeof window !== "undefined") {
          sessionStorage.removeItem("emailHeader")
        }
      }

      // Process the header
      const parsedData = parseEmailHeader(decodedHeader)

      // Check if there was an error parsing
      if (parsedData.error) {
        setError(`Error analyzing header: ${parsedData.error}`)
      }
      ...
      console.error("Error in analyze page:", err)
      setError("An error occurred while analyzing the email header. Please try again with a different header.")
      setLoading(false)
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen relative text-slate-100 flex items-center justify-center">
        ...
        <div className="text-cyan-400 text-xl font-semibold animate-pulse">Analyzing email header...</div>
      </div>
    )
  }
            ...
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-slate-400 mb-6">
                  This could be due to an invalid or malformed email header. Please try again with a different header.
                </p>
                <Link href="/">
